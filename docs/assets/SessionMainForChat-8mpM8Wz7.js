...
import { u as K } from "./useLLM-Byfd1lHl.js";
import { a as Y } from "./analytics-CvRoWAme.js";
    ...
    const m = N({
        ...
        async saveNewContextItemsToDB (e, a) {
            const { SessionDB: o } = await b(async ()=>{
                const { SessionDB: u } = await import("./db-master-DD6OhUmP.js").then((w)=>w.v);
                return {
                    SessionDB: u
                };
        ...
        const o = K(), i = d.useRef(!1), u = F(), w = d.useCallback(async (p)=>{
            if (!(i.current || !a || a.getHistoryCount() > 0 || !e)) try {
                ...
                const { makeChatModeStartingContextItems_CN: x } = await b(async ()=>{
                    const { makeChatModeStartingContextItems_CN: r } = await import("./PlayerInputItem-X8k76-AI.js").then((n)=>n.c);
                    return {
            ...
            if (!(i.current || !e || !a)) try {
                ...
                const { makeChatModeLLMMessagesFromContextStateForCharacterId: g } = await b(async ()=>{
                    const { makeChatModeLLMMessagesFromContextStateForCharacterId: l } = await import("./PlayerInputItem-X8k76-AI.js").then((C)=>C.c);
                    return {
